view: order_items_base {
  derived_table: {
    sql: SELECT
      order_items.ticketid AS transaction_id
      ,timestamp(order_items.date_closed) AS order_created_time
      ,order_items.product_id as SKU_id
      ,order_items.productname AS SKU_name
      ,order_items.retail_brand AS brand_name
      ,order_items.product_type AS type_name
      ,order_items.customer_uuid AS user_id
      ,order_items.net_sales AS sale_amt
      ,SUM(net_sales) - SUM(cost_without_excise) AS margin_amt
      FROM `fivetran-purple-lotus-warehous.dbt.core_domo_ticket_items` as order_items
  ...
  view: order_items {
    derived_table: {
      sql: SELECT
              ...
              , {% parameter order_items_base.product_level %}_name as product_id
            ...
            WHERE 1=1
            AND UPPER({% parameter order_items_base.product_level %}_name) <> ""
  ...
  view: order_purchase_affinity {
    ...
    dimension: product_a {
      ...
      link: {
        ...
        url: "/dashboards/purple_lotus::item_affinity_analysis?Focus%20Product={{ value | encode_uri }}&Product%20Level={{ _filters['order_items_base.product_level'] | url_encode }}&Analysis%20Timeframe={{ _filters['order_purchase_affinity.affinity_timeframe'] | url_encode }}"
      }
    }
...
## Do not display unless users have a solid understanding of  statistics and probability models
    ...
    dimension: product_a_average_rest_of_basket_margin {
      ...
      value_format_name: usd
      html: {{ product_a._rendered_value }} :: {{ rendered_value }} ;;
      drill_fields: [product_a, product_a_percent_purchased_alone, product_a_percent_customer_exclusivity]
    }
    ...
    dimension: product_b_average_rest_of_basket_margin {
      ...
      value_format_name: usd
      html: {{ product_b._rendered_value }} :: {{ rendered_value }} ;;
      drill_fields: [product_b, product_b_percent_purchased_alone, product_b_percent_customer_exclusivity]
    }
